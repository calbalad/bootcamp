...
public class PeliculaEditDTO {
	...
	public static PeliculaEditDTO from(Film source) {
		return new PeliculaEditDTO(source.getFilmId(), source.getTitle(), source.getDescription(),
				source.getReleaseYear(), source.getLanguage().getLanguageId(),
	...
	public Film update(Film target) {
		...
		var delActores = target.getFilmActors().stream().filter(item -> !actores.contains(item.getActor().getActorId()))
				.toList();
		delActores.forEach(item -> target.removeFilmActor(item));
		// Añade los actores que falta
		actores.stream()
				.filter(idActorDTO -> !target.getFilmActors().stream()
						.anyMatch(filmActor -> filmActor.getActor().getActorId() == idActorDTO))
		...
		var delCategorias = target.getFilmCategories().stream()
				.filter(item -> !categorias.contains(item.getCategory().getCategoryId())).toList();
		delCategorias.forEach(item -> target.removeFilmCategory(item));
		// Añade las categorias que falta
		categorias.stream()
				.filter(idCategoriaDTO -> target.getFilmCategories().stream()
						.noneMatch(filmCategory -> filmCategory.getCategory().getCategoryId() == idCategoriaDTO))
